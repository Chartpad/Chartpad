...
else
{
	...
	$start_from = ($page - 1) * $per_page; 
	$sql= mysql_query("SELECT * FROM INSE_tblTicket WHERE userID = '$userID' LIMIT $start_from, $per_page")
		  or die(mysql_error());
	$result= mysql_num_rows($sql);
	if( mysql_num_rows($sql) > 0)
	{
	...
	$total_rows = mysql_query("SELECT COUNT(*) FROM INSE_tblTicket WHERE userID = '$userID'");
	$total_rows = mysql_fetch_row($total_rows);
	$total_rows = $total_rows[0];
